...
A.FBYVERIFY AS 生成方式,
A.FOPENSTATUS AS 表头开票核销状态--T_AP_PAYABLEENTRY
FROM
T_AP_PAYABLE A
...
AND (  (C.FPAYAMOUNTFOR = 0)
    OR
    (
        (C.FPAYAMOUNTFOR > 0 AND (C.FPAYAMOUNTFOR - c.FAPPLYAMOUNT - a.FRELATEHADPAYAMOUNT + C.FPAYREAPPLYAMT) > 0)
        OR
        (C.FPAYAMOUNTFOR < 0 AND (C.FPAYAMOUNTFOR - c.FAPPLYAMOUNT - a.FRELATEHADPAYAMOUNT + C.FPAYREAPPLYAMT) < 0)
    ))
...
A.FSETACCOUNTTYPE AS 立账类型,
A.FBYVERIFY AS 生成方式,
A.FWRITTENOFFSTATUS AS 表头开票核销状态--T_AP_PAYABLEENTRY
FROM
T_AP_PAYABLE A
LEFT JOIN T_AP_PAYABLEENTRY B ON A.FID = B.FID
LEFT JOIN T_AP_PAYABLEPLAN C ON A.FID = C.FID
