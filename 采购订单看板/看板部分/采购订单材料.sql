...
MainQuery AS (
    SELECT
        ...
        CASE
            WHEN DATEDIFF(DAY, A.FAPPROVEDATE, CG.审核日期) BETWEEN 3 AND 6 THEN 1
            ELSE 0
        END AS 采购订单材料创建日期_3_6_标志,
        CASE
            WHEN DATEDIFF(DAY, A.FAPPROVEDATE, CG.审核日期) >= 7 THEN 1
            ELSE 0
	...
	LEFT JOIN CG1 CG ON CG.采购申请单单据编号 = A.FBILLNO AND CG.FMATERIALID = AA.FMATERIALID AND AA.FSEQ = CG.采购申请单分录序号
	--LEFT JOIN CG1 ON CG1.采购申请单单据编号 = A.FBILLNO AND CG1.FMATERIALID = AA.FMATERIALID AND AA.FSEQ = CG1.采购申请单分录序号

    WHERE
        A.FDOCUMENTSTATUS = 'C'
				--AND
    -- 只包括当前月份的数据
    --(CG.审核日期 >= DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) and CG.审核日期 < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0))
   --AND
    -- 只包括当前月份的数据
    --(CG1.创建日期 >= DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) and CG1.创建日期 < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0))
...
SELECT
    ...
    -- SUM(采购订单资产创建日期_3_6_标志) AS 采购订单资产创建日期_7天内_数量,
    -- SUM(采购订单资产创建日期_大于7天_标志) AS 采购订单资产创建日期_大于7天_数量
FROM MainQuery;