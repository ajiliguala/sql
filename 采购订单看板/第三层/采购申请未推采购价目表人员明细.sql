WITH CGJM AS (SELECT DISTINCT FNUMBER, FUSEORGID
              FROM (SELECT D.FNUMBER,
                    ...
                    WHERE A.FROWAUDITSTATUS = 'A'
                      AND B.FUSEORGID IN ('1', '100329', '100330', '100331', '100332', '3798064', '4355331')) AA
    --WHERE rn = 1
),
     CGSQ AS (SELECT DISTINCT a.FBILLNO             AS 单据编号,
              ...
              FROM T_PUR_Requisition A
                       LEFT JOIN T_ORG_ORGANIZATIONS_L C ON C.FORGID = A.FAPPLICATIONORGID
...
WHERE
CGJM.FNUMBER IS NULL
AND
--CGSQ.执行采购='赵圆圆'
CGSQ.执行采购='${执行采购}'
AND (
    -- 只包括当前月份的数据
    (采购申请审核日期 >= DATEFROMPARTS(YEAR(GETDATE()), MONTH(GETDATE()), 1) and 采购申请审核日期 < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0))
)
