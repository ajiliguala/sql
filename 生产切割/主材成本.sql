...
WITH CGJM AS (SELECT DISTINCT FPRICE,
     ...
     --成本计算
     CHJS AS (SELECT DISTINCT MAT.FNUMBER, tFF100001_l.FDATAVALUE, PO1.FPRICE AS 成本,PO.FDATE
...
WHERE PO.FDATE >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) - 1, 0) -- 上个月第一天
  AND PO.FDATE < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0)) -- 本月第一天
